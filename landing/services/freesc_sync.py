...
def run_scheduled_sync(*, force: bool = False) -> ReleaseSyncLog | None:
    ...
    try:
        report = sync_all_from_freesc(sync_configs=True, prune=force)
...
def format_sync_report_message(report: FreescSyncReport) -> str:
    parts = [
    ...
    for detail in report.details:
        ...
        lines.append(
            ...
            f'+{detail.created} ~{detail.updated}{extra} (на freesc.ru: {detail.total_fetched})',
        )
    return '\n'.join(lines)
